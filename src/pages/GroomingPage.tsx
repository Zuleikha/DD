...
const GroomingPage: React.FC = () => {
  ...
  // Get the appropriate generic image for each groomer
  const getGenericImage = (index: number): string => {
    return genericImages[index % genericImages.length];
  };

  const displayedGroomers = showAll ? filteredGroomers : filteredGroomers.slice(0, 6);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <SEO
        ...
        description="Find professional dog grooming services across Ireland. Expert care for your furry friend's grooming needs."
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6 mb-6">
            <div className="flex items-center space-x-2 sm:space-x-3">
              <PawPrint size={20} opacity={0.6} className="text-pink-500 sm:w-6 sm:h-6" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-8 sm:pl-10 pr-4 py-2 sm:py-3 w-full border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-pink-500 text-sm sm:text-base"
                value={searchTerm}
            ...
            <div className="relative">
              <select
                className="w-full py-2 sm:py-3 px-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-pink-500 text-sm sm:text-base"
      ...
      <section className="py-8 sm:py-12 md:py-16 relative overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none text-gray-200">
          <div className="absolute top-8 sm:top-12 md:top-16 left-8 sm:left-12 md:left-16">
            <Bone size={35} opacity={0.2} className="text-pink-200 sm:w-10 sm:h-10 md:w-12 md:h-12" />
          </div>
          <div className="absolute top-20 sm:top-32 md:top-40 right-12 sm:right-16 md:right-20">
            <PawPrint size={30} opacity={0.2} className="text-pink-200 sm:w-8 sm:h-8 md:w-10 md:h-10" />
          </div>
          <div className="absolute bottom-20 sm:bottom-32 md:bottom-40 left-1/4">
            <PawPrint size={40} opacity={0.2} className="text-pink-200 sm:w-11 sm:h-11 md:w-14 md:h-14" />
          </div>
          <div className="absolute bottom-8 sm:bottom-12 md:bottom-16 right-8 sm:right-12 md:right-16">
            <Bone size={25} opacity={0.2} className="text-pink-200 sm:w-7 sm:h-7 md:w-8 md:h-8" />
          </div>
        </div>
          ...
          {filteredGroomers.length > 0 ? (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
                {displayedGroomers.map((groomer, index) => (
                  <div key={groomer.id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow overflow-hidden group">
                    <div className="relative">
                      <img
                      ...
                      <div className="absolute top-2 sm:top-4 right-2 sm:right-4 bg-white rounded-full p-1 sm:p-2 shadow-md">
                        <PawPrint size={16} opacity={0.8} className="text-pink-500 sm:w-5 sm:h-5" />
                      </div>
                    </div>
                    
                    <div className="p-4 sm:p-6">
                      <div className="flex items-start justify-between mb-2 sm:mb-3">
                        <h3 className="text-lg sm:text-xl font-bold text-gray-800 group-hover:text-pink-600 transition-colors">
                          {groomer.name}
                        </h3>
                        <div className="flex items-center space-x-1 bg-yellow-50 px-2 py-1 rounded-full">
                          <Star className="w-3 h-3 sm:w-4 sm:h-4 text-yellow-400 fill-current" />
                          <span className="text-xs sm:text-sm font-medium text-yellow-700">{groomer.rating}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start space-x-2 mb-2 sm:mb-3">
                        ...
                        <p className="text-sm sm:text-base text-gray-600">{groomer.address}</p>
                      </div>
                      
                      <div className="inline-block bg-pink-100 text-pink-800 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium mb-3 sm:mb-4">
                        {groomer.county}
                      </div>
                      ...
                      {groomer.services && groomer.services.length > 0 && (
                        <div className="flex flex-wrap gap-1 sm:gap-2 mb-3 sm:mb-4">
                          {groomer.services.slice(0, 2).map((service, idx) => (
                            <span key={idx} className="inline-flex items-center bg-pink-50 text-pink-700 px-2 py-1 rounded-full text-xs">
                              <Bone size={12} className="mr-1" />
                              {service}
                            </span>
                          ))}
                          {groomer.services.length > 2 && (
                            <span className="text-xs text-gray-500">+{groomer.services.length - 2} more</span>
                          )}
                        </div>
                      )}
                      ...
                      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                        <Link
                          to={`/grooming/${groomer.id}`}
                          className="flex-1 bg-pink-600 text-white px-3 sm:px-4 py-2 sm:py-3 rounded-lg hover:bg-pink-700 transition-colors text-center text-sm sm:text-base font-medium"
                        >
                          View Details
                        </Link>
                        {groomer.phone && (
              ...
              {hasMoreGroomers && !showAll && (
                <div className="text-center mt-8 sm:mt-12">
                  <button
                    onClick={() => setShowAll(true)}
                    className="inline-flex items-center bg-pink-600 text-white px-6 sm:px-8 py-3 sm:py-4 rounded-lg hover:bg-pink-700 transition-colors group text-base sm:text-lg font-semibold"
                  >
                    <PawPrint size={18} className="mr-2 sm:mr-3 group-hover:animate-bounce sm:w-5 sm:h-5" />
                    Show More Groomers ({filteredGroomers.length - 6} more)
                    <ChevronDown size={18} className="ml-2 sm:ml-3 group-hover:animate-bounce sm:w-5 sm:h-5" />
                  </button>
                </div>
              )}

              {showAll && hasMoreGroomers && (
                <div className="text-center mt-8 sm:mt-12">
                  <button
            ...
            <div className="bg-gray-100 p-6 sm:p-8 rounded-lg text-center">
              <h3 className="text-lg sm:text-xl font-semibold mb-2">No groomers found</h3>
