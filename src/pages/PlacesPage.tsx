...
const PlacesPage: React.FC = () => {
  ...
  const getGenericImage = (index: number): string => {
    const images = [placeImage1, placeImage2, placeImage3, placeImage4, placeImage5, placeImage6];
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <SEO
        ...
        description="Discover dog-friendly restaurants, cafes, pubs, and attractions across Ireland. Find welcoming places for you and your furry companion."
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
            <div className="flex items-center space-x-2 sm:space-x-3">
              <PawPrint size={20} opacity={0.6} className="text-amber-500 sm:w-6 sm:h-6" />
            ...
            <div className="w-full sm:w-auto">
              <select
                ...
                className="w-full sm:w-64 px-3 sm:px-4 py-2 sm:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 text-sm sm:text-base"
              >
                {counties.map((county) => (
      ...
      <section className="py-8 sm:py-12 md:py-16 relative overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none text-gray-200">
          <div className="absolute top-8 sm:top-12 md:top-16 left-8 sm:left-12 md:left-16">
            <Bone size={35} opacity={0.2} className="text-amber-200 sm:w-10 sm:h-10 md:w-12 md:h-12" />
          </div>
          <div className="absolute top-20 sm:top-32 md:top-40 right-12 sm:right-16 md:right-20">
            <PawPrint size={30} opacity={0.2} className="text-amber-200 sm:w-8 sm:h-8 md:w-10 md:h-10" />
          </div>
          <div className="absolute bottom-20 sm:bottom-32 md:bottom-40 left-1/4">
            <PawPrint size={40} opacity={0.2} className="text-amber-200 sm:w-11 sm:h-11 md:w-14 md:h-14" />
          </div>
          <div className="absolute bottom-8 sm:bottom-12 md:bottom-16 right-8 sm:right-12 md:right-16">
            <Bone size={25} opacity={0.2} className="text-amber-200 sm:w-7 sm:h-7 md:w-8 md:h-8" />
          </div>
        </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
              ...
              <div key={place.id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow overflow-hidden group">
                <div className="relative">
                  ...
                  <div className="absolute top-2 sm:top-4 right-2 sm:right-4 bg-white rounded-full p-1 sm:p-2 shadow-md">
                    <PawPrint size={16} opacity={0.8} className="text-amber-500 sm:w-5 sm:h-5" />
                  </div>
                  {place.dogPolicy && (
                ...
                <div className="p-4 sm:p-6">
                  ...
                  {place.dogAmenities && place.dogAmenities.length > 0 && (
                    <div className="flex flex-wrap gap-1 sm:gap-2 mb-3 sm:mb-4">
                      {place.dogAmenities.slice(0, 2).map((amenity, idx) => (
                        <span key={idx} className="inline-flex items-center bg-amber-50 text-amber-700 px-2 py-1 rounded-full text-xs">
                          <Bone size={12} className="mr-1" />
                          {amenity}
                        </span>
                      ))}
                      {place.dogAmenities.length > 2 && (
                        <span className="text-xs text-gray-500">+{place.dogAmenities.length - 2} more</span>
                      )}
                    </div>
                  )}
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                    <Link
                      to={`/places/${place.id}`}
                      className="flex-1 bg-amber-600 text-white px-3 sm:px-4 py-2 sm:py-3 rounded-lg hover:bg-amber-700 transition-colors text-center text-sm sm:text-base font-medium"
                    >
                      View Details
                    </Link>
                    {place.phone && (
          ...
          {hasMorePlaces && !showAll && (
            <div className="text-center mt-8 sm:mt-12">
              <button
                onClick={() => setShowAll(true)}
                className="inline-flex items-center bg-amber-600 text-white px-6 sm:px-8 py-3 sm:py-4 rounded-lg hover:bg-amber-700 transition-colors group text-base sm:text-lg font-semibold"
              >
                <PawPrint size={18} className="mr-2 sm:mr-3 group-hover:animate-bounce sm:w-5 sm:h-5" />
                Show More Places ({filteredPlaces.length - 6} more)
                <ChevronDown size={18} className="ml-2 sm:ml-3 group-hover:animate-bounce sm:w-5 sm:h-5" />
              </button>
            </div>
          )}

          {showAll && hasMorePlaces && (
            <div className="text-center mt-8 sm:mt-12">
              <button
