...
const VetsPage: React.FC = () => {
  ...
  const getGenericImage = (index: number): string => {
    const images = [vetImage1, vetImage2, vetImage3, vetImage4, vetImage5];
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <SEO
        title="Veterinary Services in Ireland | DogDays.ie"
        description="Find trusted veterinary services across Ireland. Comprehensive directory of vets, emergency clinics, and specialized animal healthcare providers."
        canonicalUrl="https://www.dogdays.ie/vets"
      />

      {/* Responsive Hero Section */}
      <section className="relative bg-gradient-to-br from-blue-600 via-teal-500 to-blue-700 text-white py-12 sm:py-16 md:py-20 lg:py-24 xl:py-32 overflow-hidden min-h-[60vh] sm:min-h-[70vh] md:min-h-[80vh] lg:min-h-[90vh] flex items-center">
        {/* Hero Background Image */}
        <div 
          className="absolute inset-0 bg-cover bg-center bg-no-repeat"
          style={{
            backgroundImage: `url(${vetHero})`
          }}
        ></div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="text-center">
            ...
            <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl mb-6 sm:mb-8 md:mb-10 max-w-2xl md:max-w-4xl mx-auto leading-relaxed">
              Find trusted veterinary care across Ireland for your beloved companion
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
            <div className="flex items-center space-x-2 sm:space-x-3">
              <PawPrint size={20} opacity={0.6} className="text-blue-500 sm:w-6 sm:h-6" />
            ...
            <div className="w-full sm:w-auto">
              <select
                ...
                className="w-full sm:w-64 px-3 sm:px-4 py-2 sm:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm sm:text-base"
              >
                {counties.map((county) => (
      ...
      <section className="py-8 sm:py-12 md:py-16 relative overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none text-gray-200">
          <div className="absolute top-8 sm:top-12 md:top-16 left-8 sm:left-12 md:left-16">
            <Bone size={35} opacity={0.2} className="text-blue-200 sm:w-10 sm:h-10 md:w-12 md:h-12" />
          </div>
          <div className="absolute top-20 sm:top-32 md:top-40 right-12 sm:right-16 md:right-20">
            <PawPrint size={30} opacity={0.2} className="text-blue-200 sm:w-8 sm:h-8 md:w-10 md:h-10" />
          </div>
          <div className="absolute bottom-20 sm:bottom-32 md:bottom-40 left-1/4">
            <PawPrint size={40} opacity={0.2} className="text-blue-200 sm:w-11 sm:h-11 md:w-14 md:h-14" />
          </div>
          <div className="absolute bottom-8 sm:bottom-12 md:bottom-16 right-8 sm:right-12 md:right-16">
            <Bone size={25} opacity={0.2} className="text-blue-200 sm:w-7 sm:h-7 md:w-8 md:h-8" />
          </div>
        </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
            {displayedVets.map((vet: Vet, index: number) => (
              <div key={vet.id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow overflow-hidden group">
                <div className="relative">
                  ...
                  <div className="absolute top-2 sm:top-4 right-2 sm:right-4 bg-white rounded-full p-1 sm:p-2 shadow-md">
                    <PawPrint size={16} opacity={0.8} className="text-blue-500 sm:w-5 sm:h-5" />
                  </div>
                </div>
                ...
                <div className="p-4 sm:p-6">
                  <div className="flex items-start justify-between mb-2 sm:mb-3">
                    ...
                    {vet.rating && (
                      <div className="flex items-center space-x-1 bg-yellow-50 px-2 py-1 rounded-full">
                        ...
                        <span className="text-xs sm:text-sm font-medium text-yellow-700">{vet.rating}</span>
                      </div>
                    )}
                  </div>
                  ...
                  {vet.address && (
                    <div className="flex items-start space-x-2 mb-2 sm:mb-3">
                      ...
                      <p className="text-sm sm:text-base text-gray-600">{vet.address}</p>
                    </div>
                  )}
                  
                  {vet.county && (
                    <div className="inline-block bg-blue-100 text-blue-800 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium mb-3 sm:mb-4">
                      {vet.county}
                    </div>
                  )}
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                    <Link
                      to={`/vets/${vet.id}`}
                      className="flex-1 bg-blue-600 text-white px-3 sm:px-4 py-2 sm:py-3 rounded-lg hover:bg-blue-700 transition-colors text-center text-sm sm:text-base font-medium"
                    >
                      View Details
                    </Link>
                    {vet.phone && (
          ...
          {hasMoreVets && !showAll && (
            <div className="text-center mt-8 sm:mt-12">
              <button
                ...
                className="inline-flex items-center bg-blue-600 text-white px-6 sm:px-8 py-3 sm:py-4 rounded-lg hover:bg-blue-700 transition-colors group text-base sm:text-lg font-semibold"
              >
                <PawPrint size={18} className="mr-2 sm:mr-3 group-hover:animate-bounce sm:w-5 sm:h-5" />
                Show More Vets ({filteredVets.length - 6} more)
